/******************************************************************************
* File Name          : genfield_idx_v_struct.h
...
#ifndef __GENFIELD_IDX_V_STRUCT
#define __GENFIELD_IDX_V_STRUCT

/* Hardware configuration option bit assignments. */
#define AUX1PRESENT   (1 << 0)  // 1 = Genfield #1 has auxilary contacts
#define AUX1SENSE     (1 << 1)  // 1 = Aux I/O bit with Genfield #1 closed
#define AUX2PRESENT   (1 << 2)  // 1 = Genfield #2 has auxilary contacts
#define AUX2SENSE     (1 << 3)  // 1 = Aux I/O bit with Genfield #2 closed
#define ONEGENFIELD  (1 << 4)  // 1 = One genfield; one small pre-charge relay
#define PWMGENFIELD1 (1 << 5)  // 1 = PWM'ing is used on coil #1
#define PWMGENFIELD2 (1 << 6)  // 1 = PWM'ing is used on coil #2
...
#define	IDXHV1  0 // High voltage reading: battery string side of genfield
#define	IDXHV2  1 // High voltage reading: DMOC+ side of genfield
#define	IDXHV3  2 // High voltage reading: across pre-charge resistor (two genfields)
...
/* Genfield #1 closure time delay */
	uint32_t close1_t;   // Delay: genfield #1 coil energize->closed

/* Genfield #2, or pre-charge relay, closure time delay */
	uint32_t close2_t;   // Delay: #2 coil energize->closed
   
/* Genfield #1 open time delay */
	uint32_t open1_t;   // Delay: genfield #1 coil de-energize->open

/* Genfield #2, or pre-charge relay, open time delay */
...
/* Message timings. */
	...
	uint32_t hbct3_t;		// Heartbeat ct: ticks between sending msgs hv3 (if two genfields)
...
/* Send CAN ids  */
   ...
   uint32_t cid_msg1;   // CANID-genfield poll response msg: volt1:cur1 (volts:amps)
   uint32_t cid_msg2;   // CANID-genfield poll response msg: volt2:cur2 (volts:amps)
